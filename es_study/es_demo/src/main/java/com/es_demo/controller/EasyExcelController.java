...
public class EasyExcelController {
    ...
    public void exportMemberList(HttpServletResponse response) throws IOException, InterruptedException {
        ...
        log.info("当前线程池的核心线程数为----------："+corePoolSize);
        ...
        try {
            ...
            for (int sheetIndex = 0; sheetIndex < targetSheets; sheetIndex++) {
                final int currentSheetIndex = sheetIndex; // 创建 final 副本
                final int pageNum = currentSheetIndex + 1;
                final long offset = currentSheetIndex * pageSize;
                ...
                executor.submit(() -> {
                    try {
                        ...
                        if (!records.isEmpty()) {
                            dataMap.put(currentSheetIndex, records);
        ...
        try (ExcelWriter excelWriter = EasyExcel.write(response.getOutputStream(), Member.class)
                .registerWriteHandler(new LongestMatchColumnWidthStyleStrategy()) // 保留列宽自适应
                .build()) {
